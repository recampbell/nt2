...
NT2_TEST_CASE_TPL( unaligned_load,  BOOST_SIMD_SIMD_TYPES)
{
  ...
  typedef typename
          boost::dispatch::
          meta::call<unaligned_load_(T*,boost::dispatch::meta::as_<vT>)>::type r_t;
...
NT2_TEST_CASE_TPL( unaligned_load_logical,  BOOST_SIMD_SIMD_TYPES)
{
  using boost::simd::unaligned_load;
  using boost::simd::tag::unaligned_load_;
  ...
  logical<T>  data[cardinal_of<vT>::value*3];
  rT          ref[3];
  ...
  {
    rT v = boost::simd::unaligned_load<vT>(&data[i*cardinal_of<vT>::value]);
...
NT2_TEST_CASE_TPL( unaligned_load_offset_logical,  BOOST_SIMD_SIMD_TYPES)
{
  using boost::simd::unaligned_load;
  using boost::simd::tag::unaligned_load_;
  using boost::simd::logical;
  ...
  for(std::size_t i=0;i<3;++i)
  {
    rT v = boost::simd::unaligned_load<vT>(&data[0],i*cardinal_of<vT>::value);
    NT2_TEST_EQUAL(v , ref[i]);
  }
}
